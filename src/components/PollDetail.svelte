...
<div class="poll">
    ...
    <div class="answer" on:click={()=>handleVote('a',poll.id)}>
        <div class="percent percent-a" style="width:{precentA}%"></div>
        <span>{poll.answerA} ({poll.votesA})</span>
    </div>
    <div class="answer" on:click={()=>handleVote('b',poll.id)}>
        <div class="percent percent-b" style="width:{precentB}%"></div>
        <span>{poll.answerB} ({poll.votesB})</span>
    </div>
    <div class="delete">
        <Button theme='secondary' flat={true} on:click={()=>handleDelete(poll.id)}>delete</Button>
    </div>
</div>
</Card>
